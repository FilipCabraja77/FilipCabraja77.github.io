...
(function () {
  ...
  /* -------------------------------------------------------
     3. Akordeon (usluge, cjenik, FAQ) - animirano otvaranje
  ...
  /* -------------------------------------------------------
     3.0b Cjenik - tabovi na desktopu, akordeon na mobitelu

     Jedan te isti markup radi u oba načina; mijenjaju se samo uloge i
     poredak. Prekidač je matchMedia('(max-width: 768px)'), isti prag na
     kojem CSS mijenja izgled.

     Desktop (tabovi): role="tab"/"tabpanel", aria-selected, roving tabindex
     (aktivan tab jedini je u redoslijedu tabulatora), strelice lijevo/desno
     s omotom na krajevima i Home/End. Odabir slijedi fokus - panel se
     prikazuje odmah, pa potvrda Enterom nema što dodati.

     Mobitel (akordeon): uloge tabova se skidaju, gumbi dobivaju
     aria-expanded i svi su u redoslijedu tabulatora, paneli postaju
     role="region". Otvaranje i zatvaranje vode expand/collapse iz akordeona
     (točka 3), pa je kretanje isto kao u FAQ-u. Otvorena je najviše jedna
     stavka; ponovni tap zatvara.

     Cijena je jedan jedini element po usluzi (bez dvostrukog [TBD]): na
     desktopu stoji u zaglavlju panela, a u akordeonu je JS premješta u gumb
     da bude vidljiva i kad je stavka zatvorena.
     ------------------------------------------------------- */
  function pricing(rootSel) {
    var root = $(rootSel);
    if (!root) return;
    var list = $('.ptabs__list', root);
    if (!list) return;
    var btns = $$('.ptabs__tab', list);
    if (!btns.length) return;

    var listLabel = list.getAttribute('aria-label') || '';

    var items = btns.map(function (btn) {
      var panel = document.getElementById(btn.getAttribute('aria-controls'));
      return {
        btn: btn,
        panel: panel,
        head: panel ? $('.ptabs__head', panel) : null,
        chip: panel ? $('.chip--price', panel) : null,
        plus: $('.ptabs__plus', btn)
      };
    }).filter(function (it) { return it.panel; });
    if (!items.length) return;

    // Zadnja odabrana usluga - prenosi se pri promjeni načina.
    var current = items.filter(function (it) {
      return it.btn.getAttribute('aria-selected') === 'true';
    })[0] || items[0];
    var isMobile = null;

    /* --- desktop --- */
    function selectTab(item, focus) {
      items.forEach(function (it) {
        var on = it === item;
        it.btn.setAttribute('aria-selected', on ? 'true' : 'false');
        it.btn.tabIndex = on ? 0 : -1;
        it.panel.hidden = !on;
      });
      current = item;
      if (focus) item.btn.focus();
    }

    /* --- mobitel --- */
    function openItem(item) {
      items.forEach(function (it) {
        if (it === item || it.btn.getAttribute('aria-expanded') !== 'true') return;
        it.btn.setAttribute('aria-expanded', 'false');
        collapse(it.panel);
      });
      item.btn.setAttribute('aria-expanded', 'true');
      if (item.panel.hidden) expand(item.panel);
      current = item;
    }
    ...
    function toMobile() {
      ...
      items.forEach(function (it) {
        if (it.chip && it.plus) it.btn.insertBefore(it.chip, it.plus);
        it.btn.removeAttribute('role');
        it.btn.removeAttribute('aria-selected');
        it.btn.tabIndex = 0;
        it.panel.setAttribute('role', 'region');
        it.panel.removeAttribute('tabindex');
        it.panel.style.maxHeight = '';
        it.panel.style.opacity = '';

        var on = it === current;
        it.btn.setAttribute('aria-expanded', on ? 'true' : 'false');
        it.panel.hidden = !on;
        // Otvoreni panel pripremi kao i accordion(): bez fiksne visine,
        // da se prvo zatvaranje može animirati iz stvarne visine.
        if (on && !reduceMotion) it.panel.style.maxHeight = 'none';
      });
    }
    ...
    function toDesktop() {
      ...
      items.forEach(function (it) {
        if (it.chip && it.head) it.head.appendChild(it.chip);
        it.btn.setAttribute('role', 'tab');
        it.btn.removeAttribute('aria-expanded');
        it.panel.setAttribute('role', 'tabpanel');
        it.panel.tabIndex = 0;
        // Ostaci animacije akordeona - inače bi panel na desktopu ostao
        // zaključan na izmjerenu visinu.
        it.panel.style.maxHeight = '';
        it.panel.style.opacity = '';
      });
    ...
    items.forEach(function (item, i) {
      item.btn.addEventListener('click', function () {
        if (!isMobile) { selectTab(item, false); return; }
        if (item.btn.getAttribute('aria-expanded') === 'true') closeItem(item);
        else openItem(item);
      });

      item.btn.addEventListener('keydown', function (e) {
        if (isMobile) return;   // u akordeonu su gumbi obični gumbi
        var next = null;
